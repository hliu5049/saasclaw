                ...
                >
                  ...
                  <SelectContent>
                    {activeModels.map((model) => (
                      <SelectItem key={model.id} value={`${model.provider.toLowerCase()}/${model.models[0]}`}>
...
import { Plus, Trash2, Play, Square, Settings, Bot } from "lucide-react"

export function Agents() {
  const navigate = useNavigate()
  const { agents, addAgent, deleteAgent, toggleStatus } = useAgents()
  ...
  const handleToggleStatus = (id: string, currentStatus: string) => {
    toggleStatus(id)
    toast.success(`Agent ${currentStatus === "running" ? "stopped" : "started"} successfully`)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>Create New Agent (Step 1)</DialogTitle>
              <DialogDescription>
                Set up the basic identity and behavior of your agent. You can configure advanced capabilities later.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div className="space-y-2">
          ...
          <Card key={agent.id} className="flex flex-col">
            <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
              <div className="space-y-1">
                ...
                <CardDescription className="truncate max-w-[200px]" title={agent.persona}>
                  {agent.persona}
                </CardDescription>
              </div>
              <Badge variant={agent.status === "running" ? "success" : agent.status === "error" ? "destructive" : "secondary"}>
                {agent.status}
              </Badge>
            </CardHeader>
            <CardContent className="flex-1 space-y-4">
              <div className="text-sm">
                <span className="text-zinc-500">Model:</span> <span className="font-medium">{getModelName(agent.modelId)}</span>
              </div>
              <div className="flex gap-1 flex-wrap">
                {agent.config.mcp && <Badge variant="outline" className="text-[10px]">MCP</Badge>}
                {agent.config.skill && <Badge variant="outline" className="text-[10px]">Skill</Badge>}
                {agent.config.memory && <Badge variant="outline" className="text-[10px]">Memory</Badge>}
                {agent.config.rag && <Badge variant="outline" className="text-[10px]">RAG</Badge>}
                {!agent.config.mcp && !agent.config.skill && !agent.config.memory && !agent.config.rag && (
                  <span className="text-xs text-zinc-400 italic">No advanced capabilities</span>
                )}
              </div>
              <div className="text-sm text-zinc-500">
                Tasks Executed: {agent.tasksExecuted}
              </div>
            </CardContent>
            <CardFooter className="flex justify-end gap-2 border-t pt-4">
              <Button
              ...
              >
                {agent.status === "running" ? (
                  <><Square className="mr-2 h-4 w-4 text-zinc-500" /> Stop</>
                ) : (
              ...
              <Button variant="ghost" size="sm" onClick={() => handleDelete(agent.id)} className="text-red-500 hover:text-red-600 hover:bg-red-50">
                <Trash2 className="mr-2 h-4 w-4" /> Delete
              </Button>
            </CardFooter>
          </Card>
