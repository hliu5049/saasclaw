...
export function Models() {
  const { models, addModel, updateModel, deleteModel, toggleStatus } = useModels()
  const [isAddOpen, setIsAddOpen] = useState(false)
  const [isEditOpen, setIsEditOpen] = useState(false)
  const [editingModel, setEditingModel] = useState<Model | null>(null)
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    addModel({ ...formData, status: "active" })
    setIsAddOpen(false)
  ...
  const handleToggleStatus = (id: string, currentStatus: string) => {
    toggleStatus(id)
    toast.success(`Model ${currentStatus === "active" ? "disabled" : "enabled"} successfully`)
  }

  const openEdit = (model: Model) => {
    setEditingModel(model)
    setFormData({ name: model.name, baseUrl: model.baseUrl, apiKey: model.apiKey })
    setIsEditOpen(true)
  }
          ...
          <Card key={model.id} className="flex flex-col">
            <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
              <div className="space-y-1">
                ...
                <CardDescription className="truncate max-w-[200px]" title={model.baseUrl}>
                  {model.baseUrl}
                </CardDescription>
              </div>
